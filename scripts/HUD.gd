...
func _on_pause_pressed():
	...
	# Пытаемся вызвать метод toggle_pause у текущей сцены (обычно Field.gd)
	var scene = get_tree().current_scene
	if scene and scene.has_method("toggle_pause"):
		scene.toggle_pause()
	else:
		# Если метод не найден, загружаем экран паузы вручную
		var pause_scene = load("res://scenes/MenuScenes/PauseScreen.tscn")
		if pause_scene:
...
func _layout_joystick_bottom_left(c: MarginContainer, edge_inset_left: float = 0.0):
	...
	c.offset_left = edge_inset_left; c.offset_top = -260.0; c.offset_right = 260.0 + edge_inset_left; c.offset_bottom = 0.0
	c.add_theme_constant_override("margin_left", 60); c.add_theme_constant_override("margin_bottom", 100)
...
func _layout_joystick_bottom_right(c: MarginContainer):
	...
	c.offset_left = -320.0; c.offset_top = -260.0; c.offset_right = -60.0; c.offset_bottom = 0.0
	c.add_theme_constant_override("margin_right", 60); c.add_theme_constant_override("margin_bottom", 100)
	c.modulate.a = 1.0 if get_tree().has_group("tutorial") else 0.3
