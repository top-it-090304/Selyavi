...
signal use_move_vector(move_vector: Vector2)
signal fire_touch()
...
func _ready():
	var original_texture = load("res://assets/scope.png") as Texture
	...
	_touch_button = $TouchScreenButton
	_fire_button = $JoystickTipArrows/FireButton
	_inner_circle = $JoystickTipArrows
...
func _input(event):
	if event is InputEventScreenTouch or event is InputEventScreenDrag:
		if _touch_button.is_pressed():
			...
			elif event is InputEventScreenDrag drag_event:
				event_position = drag_event.position
			
			var local_event_pos = event_position - get_final_transform().origin
			var raw_direction = local_event_pos - _button_center
